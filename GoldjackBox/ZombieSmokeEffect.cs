...
namespace UltimateGoldJackBoxZombieMod
{
    public class ZombieSmokeEffect : MonoBehaviour
    {
        ...
        {
            Vector2 vector = new Vector2(worldPos.x, worldPos.y + 0.6f);
            PlaySmokeSound();
            SpawnSmoke(vector, row);
            for (int i = 0; i < 6; i++)
            {
                Vector2 vector2 = Random.insideUnitCircle * 0.5f;
                SpawnSmoke(vector + vector2, row);
            }
        }
        ...
        public void PlaySubstitutionDisappear(GameObject target, int row)
        {
            ...
            Vector2 vector = new Vector2(position.x, position.y + 0.5f);
            ...
            for (int j = 0; j < 5; j++)
            {
                Vector2 vector3 = new Vector2(Random.Range(-0.4f, 0.4f), Random.Range(0.3f, 1f));
                SpawnSmoke(vector + vector3, row);
            }
            target.SetActive(false);
            Object.Destroy(target, 0.05f);
        }
        ...
        private void SpawnSmoke(Vector2 pos, int row)
        {
            ...
            instance.SetParticle(smokeParticle, pos, row);
        }
    }
}
