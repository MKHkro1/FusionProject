		...
		public static bool Prefix(Zombie __instance)
		{
			try
			{
				if (__instance.theZombieType == Plugin.theNewZombieType)
				{
					...
					if (orCreate.IsInBigJump && component.zombie.theStatus == (ZombieStatus)20)
					{
						Jackbox_c_PositionUpdate_Hook.HandleBigJumpPhysics(component.zombie, orCreate);
						return false;
					}
					if (orCreate.IsInSmallJump)
					{
						Jackbox_c_PositionUpdate_Hook.HandleSmallJumpAnimation(component.zombie, orCreate);
						return false;
					}
					Vector3 position = component.zombie.axis.position;
					if (component.zombie.theStatus != (ZombieStatus)14 && component.zombie.theStatus != (ZombieStatus)0)
					{
					...
					if (Jackbox_c_PositionUpdate_Hook._callingBase.Contains(component.zombie))
					{
						return false;
					}
					Jackbox_c_PositionUpdate_Hook._callingBase.Add(component.zombie);
