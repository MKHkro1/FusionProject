...
public static class DiamondRandomZombiePatch
{
    ...
    public static bool PreSetRandomZombie(DiamondRandomZombie __instance, ref GameObject __result)
    {
        ...
        {
            var position = __instance.axis.position;
            List<int> ids = [];
            if (Lawnf.TravelDebuff(ObsidianRandomZombie.Debuff))
            {
                for (var i = 0; i < GameAPP.resourcesManager.allZombieTypes.Count; i++)
                    if (TypeMgr.IsBossZombie((ZombieType)i) &&
                        GameAPP.resourcesManager.zombiePrefabs[GameAPP.resourcesManager.allZombieTypes[i]] is not null)
                        ids.Add(i);
            }
            else
            {
                for (var i = 0; i < GameAPP.resourcesManager.allZombieTypes.Count; i++)
                    if (GameAPP.resourcesManager.zombiePrefabs[
                            GameAPP.resourcesManager.allZombieTypes[i]] is not null &&
                        !TypeMgr.IsBossZombie((ZombieType)i) && !TypeMgr.NotRandomZombie((ZombieType)i))
                        ids.Add(i);
            }

            if (!__instance.isMindControlled)
                __result = CreateZombie.Instance.SetZombie(__instance.theZombieRow,
                    (ZombieType)ids[Random.RandomRangeInt(0, ids.Count)], __instance.transform.position.x);
            else
                __result = CreateZombie.Instance.SetZombieWithMindControl(__instance.theZombieRow,
                    (ZombieType)ids[Random.RandomRangeInt(0, ids.Count)], __instance.transform.position.x);
            Object.Instantiate(GameAPP.particlePrefab[11],
                    new Vector3(__instance.transform.position.x, position.y + 1f, 0f), Quaternion.identity).transform
...
public static class MowerPatch
{
    ...
    public static bool PreOnTriggerStay2D(Mower __instance, ref Collider2D collision)
    {
        ...
        if (gameObject.CompareTag("Zombie") && gameObject.TryGetComponent<ObsidianRandomZombie>(out var z) &&
            z.zombie is not null
            && z.zombie.theZombieRow == __instance.theMowerRow && !z.zombie.isMindControlled)
        {
            if (!z.HasMower)
            {
                var mower = __instance.gameObject;
                Object.Destroy(mower.GetComponent<BoxCollider2D>());
                Object.Destroy(mower.GetComponent<Animator>());
                Object.Destroy(mower.GetComponent<Mower>());
                mower.transform.SetParent(z.gameObject.transform.FindChild("Zombie_innerarm_hand"));
                mower.transform.localPosition = new Vector3(-0.8f, -1.6f);
        ...
        if (gameObject.CompareTag("Zombie") && gameObject.TryGetComponent<ObsidianDollZombie>(out var z2) &&
            z2.zombie is not null
...
public static class ZombiePatch
{
    ...
    public static void PostStart(Zombie __instance)
    {
        ...
        {
            if (!__instance.isMindControlled)
                CreateZombie.Instance.SetZombie(__instance.theZombieRow, (ZombieType)98,
                    __instance.transform.position.x);
            else
                CreateZombie.Instance.SetZombieWithMindControl(__instance.theZombieRow, (ZombieType)98,
                    __instance.transform.position.x);
        }

        if (__instance.TryCast<DiamondRandomZombie>() is not null && Random.RandomRangeInt(0, 9) == 1 &&
    ...
    {
        if (__instance.theZombieType is (ZombieType)99 && __instance.gameObject.TryGetComponent<ObsidianDollZombie>(
                                                           out var z)
    ...
    {
        return __instance.theZombieType is not (ZombieType)98 or (ZombieType)99;
    }
}

public class ObsidianRandomZombie : MonoBehaviour
{
    public ObsidianRandomZombie() : base(ClassInjector.DerivedConstructorPointer<ObsidianRandomZombie>())
    {
