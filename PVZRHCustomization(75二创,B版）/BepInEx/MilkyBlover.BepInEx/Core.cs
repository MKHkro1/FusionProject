...
public static class BloverPatch
{
    public static void Postfix(Blover __instance)
    {
        if (__instance.gameObject.TryGetComponent<MilkyBlover>(out var p) && Lawnf.TravelAdvanced(45))
            __instance.StartCoroutine(p.CreateStar());
    }
}
...
public static class InitBoardPatch
{
    public static void Postfix()
    {
        try
        {
            ...
            var card = Object.Instantiate(template, template.transform.parent.parent.parent.GetChild(1).GetChild(0));
            ...
            mkbBg.transform.GetChild(1).gameObject.GetComponent<TextMeshProUGUI>().text =
                PlantDataLoader.plantData[169].field_Public_Int32_1.ToString();
            if (Board.Instance is not null && (Board.Instance.boardTag.enableTravelPlant ||
            ...
            {
                Object.Destroy(card.transform.GetChild(1).gameObject);
                Object.Destroy(card.transform.GetChild(2).gameObject);
            }
        }
        catch
        {
...
[BepInPlugin("inf75.milkyblover", "MilkyBlover", "1.0")]
public class Core : BasePlugin //169
{
...
public class MilkyBlover : MonoBehaviour
{
    ...
    public IEnumerator CreateStar()
    {
        for (var i = 0; i < 10; i++)
        {
            try
            {
                if (plant is not null && !plant.IsDestroyed() && Board.Instance is not null &&
                    !Board.Instance.IsDestroyed() && Lawnf.TravelAdvanced(45))
