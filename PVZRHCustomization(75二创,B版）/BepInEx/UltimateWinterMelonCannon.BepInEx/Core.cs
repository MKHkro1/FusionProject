...
public static class Bullet_cannonPatch
{
    ...
    public static bool PreHitLand(Bullet_cannon __instance)
    {
        ...
        {
            CreateParticle.SetParticle(200, new Vector3(__instance.cannonPos.x, __instance.cannonPos.y),
                __instance.theBulletRow);
            var pos = __instance.transform.position;
            LayerMask layermask = __instance.zombieLayer.m_Mask;
            var array = Physics2D.OverlapCircleAll(new Vector2(pos.x, pos.y), 3f);
            foreach (var z in array)
                if (z is not null && z.gameObject.TryGetComponent<Zombie>(out var zombie) && !zombie.isMindControlled)
                {
                    ...
                    if (Lawnf.TravelAdvanced(UltimateWinterMelonCannon.Buff1))
                        zombie.TakeDamage(DmgType.IceAll,
                            (int)(0.05 * (zombie.theHealth + zombie.theFirstArmorHealth +
            ...
            if (Lawnf.TravelAdvanced(UltimateWinterMelonCannon.Buff2))
                Board.Instance.SetDoom(Mouse.Instance.GetColumnFromX(__instance.transform.position.x),
                    __instance.theBulletRow, false, true, default, 3600);
            __instance.Die();
            return false;
        }
...
public class Core : BasePlugin //168
{
    public override void Load()
    {
        ...
        CustomCore.RegisterCustomPlant<MelonCannon, UltimateWinterMelonCannon>(168,
            ab.GetAsset<GameObject>("UltimateWinterMelonCannonPrefab"),
            ab.GetAsset<GameObject>("UltimateWinterMelonCannonPreview"), [(915, 32)], 24, 24, 450, 1000, 60f, 1200);
        CustomCore.RegisterCustomBullet<Bullet_melonCannon>((BulletType)UltimateWinterMelonCannon.BulletId,
            ab.GetAsset<GameObject>("ProjectileCannon_UltimateWinterMelon"));
        ...
        CustomCore.AddPlantAlmanacStrings(168, "冰毁瓜加农炮(168)",
        ...
        UltimateWinterMelonCannon.Buff1 = CustomCore.RegisterCustomBuff("瓜里藏刀：每个冰毁瓜子弹对僵尸额外造成5%血量伤害",
            BuffType.AdvancedBuff,
            () => Board.Instance.ObjectExist<UltimateWinterMelonCannon>() && Lawnf.TravelUltimate(14) &&
                  Lawnf.TravelUltimate(15), 5400, null, (PlantType)168, 1, TravelBuffOptionButton.BgType.Day);
        UltimateWinterMelonCannon.Buff2 = CustomCore.RegisterCustomBuff("真正的冰毁瓜：每个冰毁瓜子弹落地时直接生成3600伤害的寒冰毁灭菇爆炸，范围全屏",
            BuffType.AdvancedBuff,
            () => Board.Instance.ObjectExist<UltimateWinterMelonCannon>() && Lawnf.TravelUltimate(14) &&
                  Lawnf.TravelUltimate(15) && Lawnf.TravelAdvanced(UltimateWinterMelonCannon.Buff1), 28800, "red",
            (PlantType)168, 1, TravelBuffOptionButton.BgType.Day);
    }
}

public class UltimateWinterMelonCannon : MonoBehaviour
{
    public UltimateWinterMelonCannon() : base(ClassInjector.DerivedConstructorPointer<UltimateWinterMelonCannon>())
    {
    ...
    public void Update()
    {
        if (GameAPP.theGameStatus is 0 && Lawnf.TravelUltimate(15) && plant is not null &&
            plant.attributeCountdown > 10f) plant.attributeCountdown = 10f;
    }
