...
public static class PrizeMgrPatch
{
    ...
    {
        if (__instance.fake && __instance.isClicked) CustomCore.PlaySound(Core.FakeTrophySound!);
    }
    ...
    public static bool RestartBoss2(PrizeMgr __instance)
    {
        ...
        Object.Instantiate(Core.FakeTrophyAnim);
...
{
    public FakeTrophyAnim() : base(ClassInjector.DerivedConstructorPointer<FakeTrophyAnim>())
    {
    ...
    public void Start()
    {
        transform.GetChild(0).GetChild(0).localScale *=
            Math.Min(Screen.width / 1920f, Screen.height / 1080f); // * (2f / 3f);
