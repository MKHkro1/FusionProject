...
public static class BigWallNutPatch
{
    ...
    public static bool PreOnTriggerStay2D(BigWallNut __instance, Collider2D collision)
    {
        if (collision.TryGetComponent<ZombieBoss2Remake>(out var boss2))
        {
            ...
            for (var i = 0; i < 20; i++)
                CreateItem.Instance.SetCoin(Mouse.Instance.GetColumnFromX(boss2.transform.position.x),
                    boss2.zombie.theZombieRow, (int)ItemType.DiamondCoin, 1);
            return false;
        }
...
public static class InGameBtnPatch
{
    ...
    [HarmonyPrefix]
    public static bool PreOnMouseUpAsButton(InGameBtn __instance)
    {
        return __instance.buttonNumber is not 3 || !Board.Instance.ObjectExist<ZombieBoss2Remake>();
    }
}
...
public static class PlantPatch
{
    ...
    public static void Crashed(Plant __instance, ref Zombie zombie)
    {
        if (zombie is not null && zombie.TryCast<ZombieBoss2>() is not null)
            CreateZombie.Instance.SetZombie(__instance.thePlantRow, ZombieType.RandomPlusZombie,
...
public static class ZombieBossPatch
{
    ...
    [HarmonyPrefix]
    public static bool AnimPutBall(ZombieBoss __instance)
    {
        return __instance.theZombieType is not ZombieType.ZombieBoss2;
    }
    ...
    {
        if (__instance.theZombieType is not ZombieType.ZombieBoss2) return;
        if (__instance.transform.TryGetComponent<ZombieBoss2Remake>(out var remake)) remake.Alive = false;
    }
    ...
    public static ZombieBoss2Remake Remake(this ZombieBoss zomboss2)
    {
        return zomboss2.GetComponent<ZombieBoss2Remake>();
    }
    ...
    {
        if (__instance.theZombieType is not ZombieType.ZombieBoss2 ||
            GameAPP.theGameStatus is not GameStatus.InGame) return true;
