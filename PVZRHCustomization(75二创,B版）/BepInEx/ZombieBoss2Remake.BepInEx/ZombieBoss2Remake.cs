...
{
    public ZombieBoss2Remake() : base(ClassInjector.DerivedConstructorPointer<ZombieBoss2Remake>())
    {
    ...
    public int[] BanList { [HideFromIl2Cpp] get; [HideFromIl2Cpp] set; } =
        [4001, 305, 306, 309, 317, 320, 323, 326, 329, 332, 335];
    ...
    public bool ThirdStage { get; set; }
    ...
    {
        zombie.revived = true;
        zombie.animDriver = gameObject.transform.FindChild("Zombie_boss_head").FindChild("Zombie_boss_driver")
        ...
        GatlingShoot = gameObject.transform.FindChild("Zombie_boss_head").FindChild("Gatling")
            .FindChild("GatlingShoot");
        ...
        foreach (var z in GameAPP.resourcesManager.allZombieTypes)
            if ((int)z >= 200 && !TypeMgr.WaterZombie(z) && !BanList.Contains((int)z))
            {
                if (TypeMgr.IsBossZombie(z) && z is not ZombieType.UltimateMachineNutZombie)
                    BossZombies.Add(z);
                else if (z is not ZombieType.SuperSubmarine && z is not ZombieType.DolphinGatlingZombie &&
                         z is not ZombieType.UltimateMachineNutZombie) UltimateZombies.Add(z);
            }
    ...
    public static int GetRandomFalseIndex(IList<bool> list)
    {
        ...
        if (falseIndices.Count is 0) return -1;

        var randomIndex = UnityEngine.Random.RandomRangeInt(0, falseIndices.Count);
    ...
    public void AnimFreeze()
    {
        ...
        // 填充所有网格位置
        for (var x = 0; x < board.columnNum; x++)
        for (var y = 0; y < board.rowNum; y++)
        ...
        {
            var createdFreezedPlant = false;

            foreach (var plant in Lawnf.Get1x1Plants(position.x, position.y))
            {
                // 跳过特定类型的植物
                ...
                {
                    var freezedPlant = GridItem.SetGridItem(
                    ...
                    // 播放冻结特效
                    var effect = plant.transform.position;
                    effect.y += 0.5f;
    ...
    {
        var b1 = CreateBullet.Instance.SetBullet(GatlingShoot!.position.x, GatlingShoot.position.y,
            zombie.targetRow - 1, BulletType.Bullet_superCherry, BulletMoveWay.Three_up, true);
        b1.normalSpeed = -9;
        b1.Damage = 300;
        var b2 = CreateBullet.Instance.SetBullet(GatlingShoot!.position.x, GatlingShoot.position.y, zombie.targetRow,
            BulletType.Bullet_superCherry, BulletMoveWay.MoveRight, true);
        b2.Damage = 300;
        b2.normalSpeed = -9;
        var b3 = CreateBullet.Instance.SetBullet(GatlingShoot!.position.x, GatlingShoot.position.y,
            zombie.targetRow + 1, BulletType.Bullet_superCherry, BulletMoveWay.Three_down, true);
        b3.Damage = 300;
        b3.normalSpeed = -9;
    }
    ...
    public void AnimPutBall()
    {
        ...
        // 计算着陆Y坐标
        var landX = ballPos.x - 1.0f;
        var landY = Mouse.Instance.GetLandY(landX, zombie.targetRow);
        ...
        // 实例化球体对象
        var boardTransform = zombie.board.transform;
        ...
        // 配置僵尸球组件
        var zombieBall = newBall.GetComponent<ZombieBall>() ?? throw new NullReferenceException();
        zombieBall.theBallRow = zombie.targetRow;
        zombieBall.boss = true;
        zombieBall.StartBigger();

        // 配置排序组
        var ballSortingGroup = newBall.GetComponent<SortingGroup>();
    ...
    public void AnimSlotMachine()
    {
        RefreshSlotMachine(Slots);
    }

    public void AnimSlotMachine_1()
    {
        RefreshSlotMachine(Slots_1);
    }

    public void AnimSlotMachineFinal()
    {
        var i = UnityEngine.Random.RandomRangeInt(0, 9);
        foreach (var sl in Slots)
        {
            foreach (var s in sl) s.active = false;
            sl[i].active = true;
        }

        var r = Slots.GetRandomItem();
        foreach (var rr in r) rr.active = false;
    ...
    public void AnimSlotMachineSummon()
    {
        ...
        List<int> results = [];
        foreach (var sl in Slots)
            for (var index = 0; index < sl.Count; index++)
                if (sl[index].active)
                {
                    results.Add(index);
                    break;
        ...
        if (zty is not ZombieType.Nothing || zty is not ZombieType.LegionZombie)
            Instantiate(GameAPP.particlePrefab[11],
                CreateZombie.Instance.SetZombie(UnityEngine.Random.RandomRangeInt(0, board.rowNum), zty).transform
                    .position + new Vector3(0f, 0.5f, 0f), Quaternion.identity, zombie.board.transform);
        for (var r = 0; r < board.rowNum; r++)
            switch (zty)
            {
                ...
                {
                    Instantiate(GameAPP.particlePrefab[11],
                        CreateZombie.Instance.SetZombie(r, zty).transform.position + new Vector3(0f, 0.5f, 0f),
                        Quaternion.identity, zombie.board.transform);
    ...
    {
        Instantiate(Core.VaseParticle, InnerHand)!.transform.localPosition = new Vector3(0, 0);
        var vases = Board.Instance.gameObject.GetComponentsInChildren<ScaryPot>();
        for (var i = vases.Count - 1; i >= 0; i--) vases[i].OnHitted();
        for (var c = board.columnNum - 4; c < board.columnNum; c++)
        for (var r = 0; r < board.rowNum; r++)
        {
            ...
            CreateParticle.SetParticle((int)ParticleType.RandomCloud,
                new Vector3(Mouse.Instance.GetBoxXFromColumn(c), Mouse.Instance.GetBoxYFromRow(r)), r);
        }
    ...
    public void BodyTakeDamage(int theDamage)
    {
        ...
        if (ThirdStage)
        {
            ...
            if (Glove.Instance is not null && HammerMgr.Instance is not null &&
                (!Glove.Instance.gameObject.activeSelf || !HammerMgr.Instance.gameObject.activeSelf))
            {
        ...
        else if (zombie.theHealth <= zombie.theMaxHealth / 3f && zombie.theMaxHealth / 6f < zombie.theHealth)
        {
            // 处理头部部件
            SetChildActive(zombie.head, 0, false);
    ...
    public ZombieType GetZombieType()
    {
        // 创建候选僵尸类型列表
        int[] commonTypes =
            [9, 10, 15, 16, 18, 21, 30, 31, 33, 35, 36, 38, 39, 50, 51, 52, 108, 112, 110, 114, 118, 119];
    ...
    public void Remove(ref HashSet<Vector2Int> boxes)
    {
        ...
        foreach (var plant in filteredPlants)
        {
            ...
            // 移除植物周围3x3区域的方块
            for (var x = plantPos.x - 1; x <= plantPos.x + 1; x++)
            for (var y = plantPos.y - 1; y <= plantPos.y + 1; y++)
    ...
    public bool Skill()
    {
        if (!ThirdStage)
        {
            ...
            if (zombie.theHealth <= zombie.theMaxHealth / 1.25f && zombie.summonCount >= 10)
            {
                SkillBungi();
                return true;
            }
    ...
    public void SkillRv()
    {
        ...
        Vector2 targetPos = new(Mouse.Instance.GetBoxXFromColumn(zombie.targetColumn),
            Mouse.Instance.GetBoxYFromRow(zombie.targetRow));

        var targetNode = zombie.animRV.transform.Find("Shadow");
        // 计算位置偏移
    ...
    {
        for (var i = 0; i < 8; i++)
            CreateItem.Instance.SetCoin(Mouse.Instance.GetColumnFromX(zombie.transform.position.x), zombie.theZombieRow,
                (int)ItemType.DiamondCoin, 1);
        if (buff) UnlockBuff();
    }
    ...
    public void UnlockBuff()
    {
        ...
        else if (adv >= 0 && !ban.Contains(adv)) GameAPP.gameAPP.GetComponent<TravelMgr>().advancedUpgrades[adv] = true;
    }
    ...
    public unsafe void UpdateEx()
    {
        ...
        IL2CPP.il2cpp_runtime_invoke(
            (IntPtr)typeof(Zombie).GetField("NativeMethodInfoPtr_Update_Protected_Virtual_New_Void_0",
                BindingFlags.NonPublic | BindingFlags.Static)!.GetValue(null)!,
            IL2CPP.Il2CppObjectBaseToPtrNotNull(zombie), null, ref exc);
        ...
        if (ThirdStage)
        {
            if (Board.Instance.theMoney > zombie.theHealth) Board.Instance.theMoney = zombie.theHealth;
        }
        ...
        if (zombie.bossStatus is BossStatus.head_idle && zombie.freezeSpeed > 0 && !zombie.ball) HeadUpdate();
        if (zombie.bossStatus is BossStatus.head_idle && zombie.freezeSpeed > 0 && zombie.ball) HeadLeaveUpdate();
    }
    ...
    public void UpdateEyeVisualization()
    {
        ...
        if (!ThirdStage) zombie.eye.transform.GetChild(zombie.ballType is 1 ? 1 : 0).gameObject.SetActive(true);
    }
